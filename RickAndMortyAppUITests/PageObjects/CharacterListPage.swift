...
struct CharacterListPage {
    ...
    func characterCell(withName name: String) -> XCUIElement {
        return app.staticTexts[name]
    }

    // Verifica se uma célula de personagem existe
    
    func hasCharacter(named name: String) -> Bool {
        return characterCell(withName: name).exists
    }
