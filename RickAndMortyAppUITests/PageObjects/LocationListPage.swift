...
struct LocationListPage {
    let app: XCUIApplication

    // Elementos de UI que queremos testar
    var navigationBar: XCUIElement {
        app.navigationBars["Locations"]
    }
    ...
    func locationCell(withName name: String) -> XCUIElement {
        return app.staticTexts[name]
    }

    // Verifica se uma célula de personagem existe
    
    func hasLocation(named name: String) -> Bool {
        return locationCell(withName: name).exists
    }
